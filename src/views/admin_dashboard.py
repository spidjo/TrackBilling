...
def admin_dashboard():
    ...
    with usage_tab:
        ...
        cursor.execute("SELECT DISTINCT user_id FROM usage_metrics WHERE tenant_id = ?", (tenant_id,))
        ...
        query = """
            SELECT usage_date, user_id, metric_type, quantity
            FROM usage_metrics
            WHERE tenant_id = ?
        """
        params = [tenant_id]
    ...
    with users_tab:
        ...
        if user_to_analyze != "All":
            cursor.execute("""
                SELECT usage_date, metric_type, quantity
                FROM usage_metrics
    ...
    with alerts_tab:
        ...
        cursor.execute("""
            SELECT u.username, p.included_units, COALESCE(SUM(um.quantity), 0)
            FROM users u
            JOIN subscriptions s ON u.id = s.user_id AND s.is_active = 1
            JOIN plans p ON s.plan_id = p.id
            LEFT JOIN usage_metrics um ON u.id = um.user_id AND um.usage_date BETWEEN DATE('now', 'start of month') AND DATE('now')
            WHERE u.tenant_id = ?
            GROUP BY u.username, p.included_units
            HAVING SUM(um.quantity) >= 0.9 * p.included_units
        """, (tenant_id,))
        ...
        cursor.execute("""
            ...
            WHERE u.tenant_id = ? AND u.id NOT IN (
                SELECT DISTINCT user_id FROM usage_metrics
